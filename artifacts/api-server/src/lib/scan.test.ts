...
test("ScanBudget navigation and network idle scale with remaining time", () => {
  const budget = new ScanBudget(100_000);
  assert.ok(budget.navigationTimeoutMs() > 40_000);
  assert.equal(budget.networkIdleTimeoutMs(), 4_000);
...
test("ScanBudget exhaust throws ScanTimeoutError", () => {
  ...
  assert.equal(budget.networkIdleTimeoutMs(), 0);
