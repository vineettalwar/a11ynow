...
async function runWithConcurrency<T>(
  items: T[],
  limit: number,
  fn: (item: T) => Promise<T extends string ? BatchPageResult : never>,
  ...
  async function worker() {
    while (idx < items.length) {
      const i = idx++;
      const item = items[i];
      results[i] = await (fn as (item: T) => Promise<BatchPageResult>)(item);
    }
  }
...
router.post("/audit/batch", async (req, res): Promise<void> => {
  ...
  req.log.info({ count: cleanUrls.length }, "Running batch accessibility audit");

  const scannedAt = new Date();

  const pages = await runWithConcurrency(cleanUrls, 3, async (url: string): Promise<BatchPageResult> => {
    try {
