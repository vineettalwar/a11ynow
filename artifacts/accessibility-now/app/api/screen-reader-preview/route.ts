...
export async function GET(req: Request) {
  ...
  if (!rawUrl) {
    return jsonErr(400, "missing_param", "url query parameter is required.");
  }
  ...
  try {
    ...
    if (!response.ok) {
      return jsonErr(502, "fetch_failed", `The page returned HTTP ${response.status}.`);
    }
    ...
    return jsonOk({ url, items });
  } catch (err) {
    logger.error({ err, url }, "Screen reader preview failed");
    return jsonErr(500, "fetch_failed", "Could not fetch or parse the page.");
  }
}
