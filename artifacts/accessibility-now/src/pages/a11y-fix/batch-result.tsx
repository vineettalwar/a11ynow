...
export default function A11yFixBatchResult() {
  ...
  async function downloadBatchPdf() {
    const ids = successPages.map((p) => p.auditId).filter(Boolean);
    ...
    try {
      ...
      a.href = url;
      a.download = `a11y-fix-site-report-${ids.length}pages.pdf`;
      a.click();
      ...
      <section className="hero-gradient pt-20 pb-8 px-4 border-b">
        <div className="container mx-auto max-w-5xl space-y-6">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-primary uppercase tracking-widest font-sans">
              A11y Fix · Whole-site results
            </p>
          </div>
        ...
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="flex flex-wrap gap-3 mb-10">
            <Button
              ...
              disabled={pdfPending || !successPages.length}
              onClick={() => void downloadBatchPdf()}
            >
              {pdfPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <FileDown className="w-4 h-4 mr-2" />}
              Download site PDF
            </Button>
            {primaryAuditId && (
              <Button asChild variant="outline" size="sm" className="[box-shadow:none]">
                <a href={a11yFixPdfUrl(primaryAuditId)} download>
