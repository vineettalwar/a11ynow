...
import { openA11yFixPrintReport } from "@/lib/a11y-fix-batch";
...
function ComplianceStatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={cn("text-xs font-semibold font-sans rounded-full", cls)}>
      {label}
    </Badge>
  );
}
          ...
          <div className="flex flex-wrap items-start justify-between gap-3 mb-2">
            ...
            <div className="flex flex-wrap gap-1.5 shrink-0">
              <Badge
                variant="outline"
                className={cn(
                  "text-[10px] font-semibold font-sans rounded-full uppercase tracking-wide",
              ...
              <Badge
                variant="outline"
                className={cn("text-[10px] font-semibold font-sans rounded-full", difficultyBadgeClass(difficulty))}
              >
        ...
        <div className="px-6 pb-6 space-y-3 bg-white">
          {violations.length === 0 ? (
            ...
            violations.map((v, idx) => (
              <ViolationFixCard key={`${v.id}-${v.wcagCriteria}-${idx}`} violation={v} />
            ))
          )}
        </div>
      )}
      ...
      <section className="hero-gradient pt-20 pb-8 px-4 border-b">
        <div className="container mx-auto max-w-5xl space-y-6">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-foreground font-sans">A11y Fix results</p>
          </div>
        ...
        <section className="py-12 px-4">
          <div className="container mx-auto max-w-3xl text-center space-y-6">
            <div role="status" aria-live="polite" className="space-y-4">
              ...
              <AuditScanTarget pageUrl={urlParam || audit?.url || ""} />
            </div>
              ...
              <div className="flex flex-wrap gap-3">
                {audit.auditId && (
                  <Button
                    ...
                    onClick={() => openA11yFixPrintReport(audit.auditId)}
                  >
                    <FileDown className="w-4 h-4 mr-2" /> Save A11y Fix PDF
                  </Button>
                )}
