...
import { ArrowRight, Code, ShieldCheck, CheckCircle2 } from "lucide-react";
...
import type { BatchScanProgress } from "@/lib/batch-scan-sse";
...
export default function A11yFixLanding() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await runA11yFixBatchScan(trimmed, intent, setBatchProgress);
      saveA11yFixBatchResult(result);
      await new Promise((r) => setTimeout(r, 700));
      router.push("/a11y-fix/batch-result");
    } catch (err) {
  ...
  const intentIcons: Record<A11yFixIntent, typeof A11yFixIcon> = {
    self: A11yFixIcon,
    engineers: Code,
    monitor: ShieldCheck,
  };
      ...
      <section ref={heroRef} className="hero-gradient pt-24 pb-16 px-4">
        <div className="container mx-auto max-w-4xl text-center space-y-8">
          ...
          <p className="text-muted-foreground text-base max-w-2xl mx-auto mb-10 reveal-body" style={{ fontFamily: "var(--app-font-mono)" }}>
            BFSG scan with issues grouped by POUR. See what to fix first, then escalate to engineers when needed.
          </p>
        </div>
      </section>
